...
tmp_dir = "tmp/"
output_dir = "/workspace/tensorrt/out/"
...
def process_videos():
    for f in files:
        ...
        mux_path = os.path.join(
            output_dir, os.path.splitext(os.path.basename(f))[0] + "_mux.mkv" # output can be an mp4
        )
        ...
        file = os.path.join(
            input_dir, os.path.splitext(os.path.basename(f))[0] + ".mp4"
        )
        if os.path.exists(mux_path):
            print(f"Processing completed for {file}. Removing original file.")
            os.remove(file)  # This line removes the original .mkv file
        else:
            print(f"Failed to process {file}. Original file retained.")
